...
6.  **配置 KV 与变量**：
    ...
    * 可选添加关键词删除变量：`FILTER_KEYWORDS`、`FILTER_CHAT_IDS`、`FILTER_NOTIFY_CHAT_IDS`、`FILTER_CASE_SENSITIVE`。
    * 点击 **Save and Deploy**。
...
5. 如需删除通知，配置 `FILTER_NOTIFY_CHAT_IDS`。机器人会先发送命中说明，再复制原消息，最后删除原消息。若通知到个人用户，该用户必须先私聊机器人并发送过 `/start`。
