...
| **🧹 频道/群关键词删除** | 可通过环境变量配置多个关键词，机器人监听目标频道/群消息，命中后直接删除，不存储消息内容。 |
...
7.  **⚠️ 关键步骤：绑定数据库与变量**
    ...
    * **添加环境变量**：
        ...
        * `FILTER_KEYWORDS`（可选）: 需要自动删除的关键词，支持多个，用英文逗号、分号或换行分隔，例如 `广告,spam,开户链接`。
        * `FILTER_BOT_IDS`（可选）: 需要直接删除其消息的机器人 ID，支持多个，用英文逗号、分号或换行分隔。
        * `FILTER_BOT_USERNAMES`（可选）: 需要直接删除其消息的机器人用户名，支持多个，可写 `soso_bot` 或 `@soso_bot`。
        * `FILTER_CHAT_IDS`（可选）: 需要监听的频道/群 ID，支持多个，用英文逗号、分号或换行分隔。建议配置，避免误删其它群消息。不配置时默认不会处理 `SUPERGROUP_ID` 管理群。
        * `FILTER_NOTIFY_TOPIC_TITLE`（可选）: 删除通知自动话题名称，默认 `删除通知`。
        * `FILTER_NOTIFY_CHAT_IDS`（可选）: 除自动通知话题外，额外接收删除通知和原消息副本的用户/群 ID，支持多个，用英文逗号、分号或换行分隔。
        * `FILTER_CASE_SENSITIVE`（可选）: 是否区分大小写，填 `true` 开启；默认不区分大小写。
...
6.  **配置 KV 与变量**：
    ...
    * 可选添加过滤变量：`FILTER_KEYWORDS`、`FILTER_BOT_IDS`、`FILTER_BOT_USERNAMES`、`FILTER_CHAT_IDS`、`FILTER_NOTIFY_TOPIC_TITLE`、`FILTER_NOTIFY_CHAT_IDS`、`FILTER_CASE_SENSITIVE`。
    * 点击 **Save and Deploy**。
...
如果需要监听频道消息，请确保 webhook 允许 `channel_post` 更新，例如：
    ```
...
1. 将机器人加入需要监听的频道或群，并设为管理员。
2. 在群/超群/频道中删除任意消息需要授予机器人 **Delete messages** 权限。
3. 关键词匹配范围包括文本消息和媒体 caption；机器人只做实时匹配和删除，不会保存消息内容。
4. 多个关键词可在 `FILTER_KEYWORDS` 中用英文逗号、分号或换行分隔。
5. 如需按发送机器人删除，配置 `FILTER_BOT_IDS` 或 `FILTER_BOT_USERNAMES`，命中后不再要求关键词匹配。
6. 删除通知会自动发送到管理群中的独立话题；如果话题不存在，机器人会自动创建，默认标题为 `删除通知`。
7. 如需额外通知用户或其它群，配置 `FILTER_NOTIFY_CHAT_IDS`。机器人会先发送命中说明，再复制原消息，最后删除原消息。若通知到个人用户，该用户必须先私聊机器人并发送过 `/start`。
