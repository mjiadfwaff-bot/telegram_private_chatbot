...
7.  **⚠️ 关键步骤：绑定数据库与变量**
    ...
    * **添加环境变量**：
        ...
        * `FILTER_NOTIFY_CHAT_IDS`（可选）: 关键词命中后接收删除通知和原消息副本的用户/群 ID，支持多个，用英文逗号、分号或换行分隔。
...
6.  **配置 KV 与变量**：
    ...
    * 可选添加过滤变量：`FILTER_KEYWORDS`、`FILTER_BOT_IDS`、`FILTER_BOT_USERNAMES`、`FILTER_CHAT_IDS`、`FILTER_NOTIFY_CHAT_IDS`、`FILTER_CASE_SENSITIVE`。
    * 点击 **Save and Deploy**。
...
6. 如需删除通知，配置 `FILTER_NOTIFY_CHAT_IDS`。机器人会先发送命中说明，再复制原消息，最后删除原消息。若通知到个人用户，该用户必须先私聊机器人并发送过 `/start`。
